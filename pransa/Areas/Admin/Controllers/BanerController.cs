    ...
    public class BanerController : BaseController {
       public BanerController(ContextHampadco _db,IWebHostEnvironment  env) :base(_db,env)
        ...
        public IActionResult list () {
            ...
            if (d != null) {
                ...
                foreach (var item in d) {
                    var q = db.Tbl_Product.Where (a => a.Id == item.CategoryProductIdSlid).FirstOrDefault ();
                    if (q ==null)
                    {
                        prname="صفحه  ی ثبت نشده";
                    }else
                    {
        ...
        public async Task<IActionResult> add (Vm_Baner ex) {
            ...
            var cc = db.Tbl_Baner.Where(a=>a.IdBaner==ex.IdBaner).FirstOrDefault();
        ...
        public IActionResult del (int id) {
            var qdel = db.Tbl_Baner.Where (add => add.Id == id).FirstOrDefault ();
