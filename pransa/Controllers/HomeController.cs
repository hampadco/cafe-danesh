...
namespace pransa.Controllers
{
    ...
    public class HomeController : BaseController
    {
        ...
        public IActionResult Index()
        {
            ...
            //تخفیف
            ViewBag.t = db.Tbl_Product.Where(a => a.MaterialPro == "تخفیف").OrderByDescending(a => a.Id).Take(4).ToList();
            //جدیدترین
            ViewBag.g = db.Tbl_Product.Where(a => a.MaterialPro == "جدیدترین").OrderByDescending(a => a.Id).Take(4).ToList();
            //پیشنهادشگفت انگیز
            ViewBag.s = db.Tbl_Product.Where(a => a.MaterialPro == "پیشنهادشگفت انگیز").OrderByDescending(a => a.Id).Take(4).ToList();
            //برجسته
            ViewBag.b = db.Tbl_Product.Where(a => a.MaterialPro == "برجسته").OrderByDescending(a => a.Id).Take(4).ToList();
            //پرفروش
            ViewBag.p = db.Tbl_Product.Where(a => a.MaterialPro == "پرفروش").OrderByDescending(a => a.Id).Take(4).ToList();
            //پیشنهادلحظه ای
            ViewBag.l = db.Tbl_Product.Where(a => a.MaterialPro == "پیشنهادلحظه ای").OrderByDescending(a => a.Id).Take(4).ToList();
            //پیشنهادما
            ViewBag.m = db.Tbl_Product.Where(a => a.MaterialPro == "پیشنهادما").OrderByDescending(a => a.Id).Take(4).ToList();
        ...
        public IActionResult search(string myinput)
        {
                       if (myinput != null)
            {
                var result = db.Tbl_Product.Where(a => a.TitleProductPro.Contains(myinput)).ToList();
                
                var mresult = map.Map<List<Vm_Product>>(result);
                return View(mresult);
            }
            return View();
        ...
        {
            var qp = db.Tbl_Product.Where(a => a.CategoryChildIdPro == id.ToString()).ToList();
            if (qp.Count() != 0)
            {
                var qpag = map.Map<List<Vm_Product>>(qp);
                TempData["cat"] = db.tbl_category.Where(a => a.Id == id).Select(a => a.NameCat).FirstOrDefault();
                return View(qpag);
            }
        ...
        public IActionResult selectcat(int id)
        {
            ViewBag.slist = db.tbl_category.Where(a => a.FatherIdCat == id).ToList();
